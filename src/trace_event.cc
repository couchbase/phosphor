...
TraceEvent::TraceEvent(const tracepoint_info* _tpi,
                       std::array<TraceArgument, arg_count>&& _args)
    : tpi(_tpi),
      args(_args),
      time(duration_cast<nanoseconds>(steady_clock::now().time_since_epoch())
                   .count()),
      duration(0) {
}
...
const std::array<TraceArgument::Type, arg_count>& TraceEvent::getArgTypes()
        const {
...
TraceEvent::ToJsonResult TraceEvent::typeToJSON() const {
    ...
    case Type::AsyncStart:
        res.type = "b";
        res.extras = utils::format_string(",\"id\": \"0x%" PRIxPTR "\"",
                                          args[0].as_pointer);
        return res;
    case Type::AsyncEnd:
        res.type = "e";
    ...
    case Type::Complete:
        ...
        res.extras =
                utils::format_string(",\"dur\":%lld.%03lld", dur_us, dur_ns);
        return res;
    }
    throw std::invalid_argument(
